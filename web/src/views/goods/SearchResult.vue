<template>
  <div>
    ...
    />
    <van-search @search="reset" v-model="query.q" show-action placeholder="请输入搜索关键词">
      <template #action>
    ...
    <van-dropdown-menu>
      ...
      <van-dropdown-item @change="reset" v-model="query.field" :options="fieldOptions" />
      <van-dropdown-item @change="reset" v-model="query.order" :options="orderOptions" />
    </van-dropdown-menu>
...
export default {
  ...
  methods: {
    ...
    async load () {
      ...
      try {
        const res = await searchGoods(this.query)
